...
public class IngredientComponent extends GuiComponent {
    ...
    public IngredientComponent(Gui gui, boolean hasItem, List<ItemInstance> inputs, ResourceName tex) {
        super(gui, 0, 0, 14, 14);
    ...
    public IngredientComponent(Gui gui, boolean hasItem, List<ItemInstance> inputs) {
    ...
    public void render(IGameInstance game, IAssetManager manager, IRenderer g, int x, int y) {
        if (!this.inputs.isEmpty()) {
            ...
            g.renderItemInGui(game, manager, input, x + 2, y + 2, 1.0F, this.hasItem ? Colors.WHITE : Colors.multiplyA(Colors.WHITE, 0.35F), true, false);
        } else {
